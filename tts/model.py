...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.style_embedding_dim = hparams.language_embedding_dim + hparams.speaker_embedding_dim + hparams.emotion_embedding_dim
        self.encoder_embedding_dim = hparams.encoder_embedding_dim + self.style_embedding_dim + hparams.token_embedding_size
    ...
    def decode(self, decoder_input, attention_weights=None, inference=False):
    ...
    def inference(self, memory, logging=True):
        """ Decoder inference
        PARAMS
        ------
        memory: Encoder outputs
        ...
        while True:
            ...
            mel_output, gate_output, alignment = self.decode(decoder_input, inference=True)
            ...
            if not logging:
                if end_trigger >= 20:
                    break
                elif (torch.sigmoid(gate_output.data) > self.gate_threshold) and end_trigger >= 15:
                    break
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.spk_adv_using = hparams.spk_adv_using
        self.spk_adv = SpeakerAdversarial(hparams)

    def parse_batch(self, batch):
        text_padded, input_lengths, mel_padded, gate_padded, \
            output_lengths, language_ids, speaker_ids, emotion_ids = batch
        ...
        return ((text_padded, input_lengths, mel_padded, max_len, output_lengths,
                 language_ids, speaker_ids, emotion_ids),
    ...
    def forward(self, inputs):
        inputs, input_lengths, targets, max_len, output_lengths, \
            language_ids, speaker_ids, emotion_ids = inputs
        input_lengths, output_lengths = input_lengths.data, output_lengths.data
        
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
        embedded_text = self.encoder(embedded_inputs, input_lengths)
        
        if self.spk_adv_using:
            spk_adv_outputs = self.spk_adv(embedded_text)
        else:
            spk_adv_outputs = None
        ...
        embedded_styles = torch.cat((embedded_language, embedded_speaker, embedded_emotion), dim=1)[:,None].repeat(1, embedded_text.size(1), 1)
        
        if np.random.uniform(0.0, 1.0) <= self.p_gst_using:
            embedded_gst = self.gst(targets)
            embedded_gst = embedded_gst.repeat(1, embedded_text.size(1), 1)
        else:
            embedded_gst = torch.zeros(inputs.size(0), embedded_text.size(1), self.token_embedding_size).cuda()
        
        encoder_outputs = torch.cat(
            (embedded_text, embedded_styles, embedded_gst), dim=2)
        ...
        return self.parse_output(
            [mel_outputs, mel_outputs_postnet, gate_outputs, alignments, spk_adv_outputs],
            output_lengths)

    def inference(self, inputs, gst_token=None, gst_mel=None, logging=True, gst_intensity=None):
        inputs, language_ids, speaker_ids, emotion_ids = inputs
        ...
        if gst_token:
            ...
            embedded_gst = self.gst.stl.attention(query, key).repeat(1, embedded_text.size(1), 1)
            if gst_intensity:
                embedded_gst = embedded_gst * gst_intensity
        elif gst_mel is not None:
            embedded_gst = self.gst(gst_mel).repeat(1, embedded_text.size(1), 1)
        else:
            embedded_gst = torch.zeros(inputs.size(0), embedded_text.size(1), self.token_embedding_size).cuda()
    ...
    def inference_noattention(self, inputs, gst_token=None, gst_mel=None):
        text, language_ids, speaker_ids, emotion_ids, attention_map = inputs
